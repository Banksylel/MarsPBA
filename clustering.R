...
dummyPrepocessing <-  function(dataset, printFlag = FALSE){
...
main<-function(){
  ...
  telcoPreprocessed <- dummyPrepocessing(telco)
  print(head(telcoPreprocessed))
  

  positionOutput<-which(names(telcoPreprocessed)==OUTPUT_FIELD)
  predictors<-telcoPreprocessed[,-positionOutput]
  
  
  
  modelKmeans <- kmeans(x=predictors, centers=4, nstart=25)
